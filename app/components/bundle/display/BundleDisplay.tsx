...
export default function BundleDisplay() {
  ...
  return (
    <AnimatePresence mode="wait">
      {selectedCookie && (
        <motion.div
          className="md:sticky h-fit md:px-8 relative top-0 flex flex-col w-full"
          initial={{opacity: 0, y: 50}}
          animate={{opacity: 1, y: 0}}
          transition={{duration: 0.6}}
        >
          <div className="md:pt-[60px] lg:pt-[72px] flex flex-col items-center xl:pt-[80px]">
            <BundleDisplayTitle />
            <motion.div
              className="aspect-square relative flex flex-col items-center justify-center flex-shrink-0 w-full overflow-hidden"
              initial={{scale: 0.9}}
              whileInView={{scale: 1}}
              viewport={{once: true}}
              transition={{duration: 0.6}}
